...
import { MISSION_META, MISSION_ORDER, getCurrentSeason, getSeasonLabel } from "@/lib/missions";
...
const TasksPage = () => {
  ...
  // --- Timely Tasks: overdue or due tasks (including recurring reminders) ---
  ...
  // --- Mission summaries (exclude recurring_reminder) ---
  const missionSummaries = useMemo(() => {
    const byMission: Record<string, { done: number; total: number; seasonalCount?: string }> = {};
    const currentSeason = getCurrentSeason();

    for (const t of tasks) {
      if (!t.mission || t.mission === "recurring_reminder") continue;
    ...
    // Add seasonal count info for seasonal_prep
    if (byMission.seasonal_prep) {
      const seasonTasks = tasks.filter(
        (t) => t.mission === "seasonal_prep" && t.season === currentSeason
      );
      const seasonLabel = currentSeason === "fall" ? "fall" : "spring";
      byMission.seasonal_prep.seasonalCount = `${seasonTasks.filter(t => t.status === "completed").length}/${seasonTasks.length} ${seasonLabel} tasks`;
    }
    ...
    for (const m of MISSION_ORDER) {
      ...
      if (s.done >= s.total) complete.push(m);
        ...
        <section className="mb-8">
          ...
          <div className="flex flex-col gap-2">
            {timelyTasks.map((task) => {
              const missionIcon = task.mission && task.mission !== "recurring_reminder"
                ? MISSION_META[task.mission]?.icon
                : "🔁";
              return (
                <motion.button
                  key={task.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-body-small text-muted-foreground truncate text-xs">
                    {missionId === "seasonal_prep" && summary?.seasonalCount
                      ? summary.seasonalCount
                      : meta.hook}
                  </p>
                </div>
